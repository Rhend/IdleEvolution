...
func _test_tirage_victoire() -> void:
	...
	var run := _run("biome_usine", 11, _butin_exact(2, 1.0))
	var vaincus: Array = [_du_bestiaire("creature_flamebot")]
	var b := run._butin_pour_victoire(vaincus)
	_check("fréquente : 2 (bornes 2-2)", int(b.get("res_ferraille", 0)) == 2)
	_check("rare : 1 (chance 100 %)", int(b.get("res_puce", 0)) == 1)
	var b2 := run._butin_pour_victoire([_du_bestiaire("creature_flamebot"),
			_du_bestiaire("creature_workbot")])
	_check("2 vaincus → fréquente ×2 (4)", int(b2.get("res_ferraille", 0)) == 4)
	_check("2 vaincus → rare ×2", int(b2.get("res_puce", 0)) == 2)
	var run0 := _run("biome_usine", 11, _butin_exact(3, 0.0))
	var b0 := run0._butin_pour_victoire(vaincus)
	_check("chance rare 0 → fréquente seule", int(b0.get("res_ferraille", 0)) == 3
			and not b0.has("res_puce"))

func _test_gates_et_absents() -> void:
	print("\n[2] Gates : hors-bestiaire, Lieu sans biome, palier sans entrée")
	var run := _run("biome_usine", 12, _butin_exact(2, 1.0))
	...
	_check("Lieu sans biome → rien",
			run_sans._butin_pour_victoire([_du_bestiaire("creature_flamebot")]).is_empty())
	var cfg_vide := ButinConfigData.new()
	cfg_vide.qte_frequente_par_palier = {}
	cfg_vide.chance_rare_par_palier = {}
	var run_vide := _run("biome_usine", 12, cfg_vide)
	_check("palier absent de la config → rien",
			run_vide._butin_pour_victoire([_du_bestiaire("creature_flamebot")]).is_empty())
...
func _test_credit_sortie() -> void:
	...
	var avant := _solde("res_ferraille")
	var run := _run("biome_usine", 4242, _butin_exact(2, 0.0), 100000.0)
	_jusqua_choix(run)
	_check("des combats ont été gagnés", run.nb_combats > 0 and not run.defaite)
	var accumule := int(run.butin_accumule.get("res_ferraille", 0))
	...
	_check("RIEN crédité avant la sortie", _solde("res_ferraille") == avant)
	run.extraire()
	_check("extraction → crédit du butin", _solde("res_ferraille") == avant + accumule)
	var recap := _dernier_recap
	_check("recap : butin_credite = accumulé",
			int((recap.get("butin_credite", {}) as Dictionary).get("res_ferraille", 0)) == accumule)

func _test_defaite_sans_credit() -> void:
	print("\n[4] Défaite = rien (mêmes rails que l'Euren)")
	var avant := _solde("res_ferraille")
	var run := _run("biome_usine", 4242, _butin_exact(2, 0.0), 1.0, 0.1)
	_jusqua_choix(run)
	_check("la run s'est soldée par une défaite", run.defaite and run.est_terminee)
	_check("défaite → aucun crédit", _solde("res_ferraille") == avant)
...
func _test_coffre() -> void:
	...
	var run := ExpeRun.new(_cfg_carte(0.0, 0.0, 1.0), _palier(), "biome_usine", 33,
...
func _test_boss_assaut() -> void:
	...
	_check("rare garantie ×2 accumulée puis créditée",
			_solde("res_puce") == avant + 2)
	_check("recap d'assaut : butin_credite porte la rare",
			int((_dernier_recap.get("butin_credite", {}) as Dictionary).get("res_puce", 0)) == 2)
