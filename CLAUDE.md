...
- **UI d'expédition** (lancement, carte, recap, Game Over) : styles UNIQUEMENT via les
  tokens `UIColors.CYBER_*` + factories `ExpeStyle` (peau cyberpunk intérimaire,
  chantier 10) — zéro littéral de style ; rouge réservé à l'Artefact/danger. ⚠ Le
  COMBAT est SORTI de ce périmètre le 07/09/2026 : DA réelle de Christophe livrée
  (`CombatUiSkin`) — le rouge y est un choix de chara design assumé pour tout le camp
  adverse (boutons/panneaux/PV/portraits), pas une régression de la règle ci-dessus.
...
- **La ShowRoom est un BANC D'ESSAI, pas une fin** (acté 2026-08-27) : tout réglage
  ...
  le pool de l'expédition. Donc : un réglage propre à un personnage va dans le registre
  `data/personnages/spine_personnages.tres` ; un réglage de présentation va dans
...
| UI de combat CTB JOUABLE (Rework ch.5 : écran scindé placeholder, file d'initiative N=6, Attaquer/Défendre + choix de cible, dégâts flottants, transitions et issue de bataille — pull-based sur un moteur démarré, `facteur_delais = 0` pour les tests ; ch.6 : issue enrichie XP/Euren via `recompenses_fournisseur` ; 07/2026 : CIBLAGE À LA SOURIS — l'ennemi se choisit en cliquant son PERSONNAGE dans la scène (zones de clic actives en mode ciblage seulement, anneau or par cible + réticule/chevron au survol — même or `SELECTION_GOLD` que les cartes) ou sa carte ; PLUS de boutons nominatifs de cible (invite + Annuler seuls) ; 08/2026 : fond scindé délégué à `CombatFondScinde` (décor RÉEL de Christophe côté joueur — `assets/background/city/`, calé sur `DECOR_SOL_FRAC` — biome placeholder `BiomeBackground` côté adverse, le Lieu n'ayant pas encore son art), SOURCE PARTAGÉE avec ShowRoom : les deux écrans ne peuvent plus diverger ; les plans de la ville sont parallaxés et défilants, voir `CombatDecorCity` ; 07/09/2026 : chrome RÉEL de Christophe posé par-dessus — boutons d'action, panneau + barre de PV de chaque carte, cadre de la file d'initiative, réticule de ciblage en scène, curseur personnalisé, splash d'ouverture (voir `CombatUiSkin`)) | `scenes/combat_ctb/CombatCtbUi.gd` + `CarteCombattantCtb.gd` + `CombatFondScinde.gd` |
| **DA réelle de combat** (livraison « Ui combat » de Christophe, 07/09/2026) : `assets/ui/Combat/*.png` (boutons, panneaux de carte, barre de PV, cadres de file d'initiative, splash d'ouverture) + `assets/ui/Icone/*.png` (réticule de ciblage, glyphe Artefact) + `assets/ui/UI_Cursor.png` — chaque famille est livrée en couches `_Back`/`_Border`/`_Aura` à FUSIONNER (mêmes dimensions par famille, jamais un fichier « fini » seul), `_composer()` les compose une fois en cache. Rouge = camp adverse assumé sur cet écran (voir plus haut). Non branché cette passe, PAS oublié : `Panel_Separator_*` (table de stats détaillée du mockup, feature absente), `Icone_Sapiens` (aucun usage identifié), `Cyber_Line` (connecteur décoratif bouton→personnage, routage dynamique reporté). L'état « ciblable » (or) reste PROCÉDURAL (aucune texture livrée, c'est un état de jeu) | `scripts/autoloads/CombatUiSkin.gd` |
...
| **Enseignes néon VIVANTES** (27/08/2026) : un point lumineux court le long du tube de chaque enseigne, comme sur la DA du QG. ⚠ SUPERSÉDÉ 29/08/2026 : Christophe livrait chaque plan en DEUX images de même cadrage (façade nue / façade avec enseignes) dont la DIFFÉRENCE isolait les enseignes — il livre désormais chaque enseigne DÉJÀ DÉCOUPÉE (calque transparent partout sauf sur le néon, alpha plein sur le tube + halo diffus autour), et un même immeuble peut porter plusieurs calques (`_Neon`, `_Neon_2`…) pour des enseignes livrées en plusieurs passes. L'extraction se fait donc désormais par SEUIL SUR LE CANAL ALPHA du calque, plus de diff ni de façade nue requise. Le reste de la mécanique est inchangé : chaque composante connexe est une enseigne, et son CONTOUR EXTÉRIEUR est déjà le tracé — les enseignes creuses (grand cadre rouge, globe filaire, triangle suspendu) sont des bandes fines dont le pourtour suit le néon, les panneaux pleins donnent le tour de leur cadre, soit le chenillard d'une vraie enseigne. AUCUNE squelettisation. Extraction hors ligne (2 passes : localisation au 1/4 puis contour fin dans chaque boîte), bakée dans `data/decor/neons_cite.tres` (25 enseignes : 19 plan 4, 6 plan 3 — `Background_City_Plan_3_Immeuble_01_Neon_2` est analysé mais n'a produit AUCUN tracé, du texte néon en traits trop fins pour le plancher de taille calibré sur les enseignes-tubes ; enregistré quand même, vide, pour le contrôle d'obsolescence — voir `bake_neons_cite.gd`). Au runtime, `NeonRunners.poser()` met un nœud dans CHAQUE copie du ruban : le sprite portant déjà `scale = affiché/source`, les points travaillent dans les pixels de l'image ET héritent gratuitement du défilement, du repli, du zoom-duel et de la brume — rien n'a été recâblé. Rendu additif, phase/vitesse tirées d'un RNG seedé par calque (reproductible, donc testable), et les enseignes trop petites à l'écran écartées par taille RENDUE, jamais par numéro de plan. ⚠ Le point NE COURT PAS en permanence : chaque enseigne suit son propre CYCLE d'allumage (`intensite_cycle` — période 6-15 s, 22-45 % allumée, fondus de 0,45 s, déphasage tiré par enseigne → ~un tiers d'actives à un instant donné) plus 12 % de tubes définitivement morts. En continu, l'œil lit une horloge et le décor paraît mécanique. ⚠ Le tracé est RECENTRÉ sur le tube au bake : le contour brut suit le bord extérieur de la forme, HALO PEINT COMPRIS, et le point courait visiblement à côté de l'enseigne. On sonde vers l'intérieur et on s'arrête au pixel le PLUS LUMINEUX (médiane sur les sommets) — chercher un maximum de lumière plutôt qu'une demi-épaisseur géométrique rend la mesure insensible à l'épaisseur du halo, qui varie d'une enseigne à l'autre. TestNeonsCite mesure le résultat : ≥ 80 % des sommets doivent tomber sur du néon allumé (91-92 % sur la livraison actuelle). ⚠ Le CŒUR du point est blanchi (`COEUR_BLANC`) : de la couleur de son tube, il est invisible sur un néon déjà saturé — mesuré, il fallait amplifier la différence 6× pour l'apercevoir ; un chenillard réel se repère parce qu'il est surexposé. Dégradation propre : bake absent/périmé/tracé dégénéré → décor strictement inchangé, sans point. Calibrage : `SHOT_MODE=neons` (ScreenshotTool) sort témoin + captures + diff amplifiée | `scenes/combat_ctb/NeonRunners.gd` + `scripts/resources/NeonsCiteData.gd` + `tools/bake_neons_cite.gd` |
| **ShowRoom** (vitrine dev des assets Spine, 08/2026) : `scenes/showroom/ShowRoom.tscn`, atteinte par le bouton dev « ShowRoom » du hub (`Village.DEBUG_SHOWROOM_BTN`, Échap revient au QG via la static `ShowRoom.scene_retour`) ou lancée seule (Échap quitte alors) ; n'écrit JAMAIS la sauvegarde. Mode LIBRE = fond neutre + repère de sol, une RANGÉE par personnage — le HÉROS d'abord (repère de mise en scène ; l'étalon de TAILLE est WorkBot depuis 09/2026, voir la ligne SpriteSpinePersonnage), puis les monstres, chacun montrant ses apparences de gauche à droite (5 paliers Commun→Légendaire pour un ennemi, **6 niveaux d'équipement Commun→Unique pour Relic**) —, caméra pan/zoom ; mode COMBAT = présentation PARTAGÉE avec CombatCtbUi (`CombatFondScinde`, même appel — pas une copie) — fond scindé en diagonale (décor réel Christophe côté joueur, biome placeholder `BiomeBackground` « forest » côté adverse), même dessin de sol (ligne d'horizon + emplacements), `SOL_Y_FRAC`/`SOL_X_*`/`HAUTEUR_ETALON_PX` recopiés — du monstre courant face au héros, `←/→` monstre, `↑/↓` palier, `H` niveau d'équipement du héros. `V` = accessoire de visage (jeux `cosmetiques` du registre, axe INDÉPENDANT du niveau — il repeuple toute la vitrine). ANIMATIONS à la demande (`TOUCHES_ANIM`) : `I` repos, `A` mêlée, `T` tir, `X` coup reçu, `M` mort — jouées sur TOUS les sprites affichés d'un coup (en mode libre, les 6 costumes attaquent ensemble) ; `reprendre_repos()` (réservé à la vitrine) sort de la pose de mort. ÉCLAIRAGE `B` (4 niveaux Nuit/Studio/Jour/Blanc, défaut Studio — le fond quasi noir d'origine noyait les paliers Commun) : il agit sur le FOND en libre et sur un VOILE posé au-dessus du décor en combat — **jamais sur les personnages** (les moduler fausserait ce qu'on vient juger). Les textes s'assombrissent au-delà de `SEUIL_FOND_CLAIR`. Contenu lu du registre `data/personnages/spine_personnages.tres` (`SpinePersonnagesData`) : **un monstre livré = une entrée, rien d'autre à toucher**. Captures : `SHOT_MODE=showroom` (ScreenshotTool) | `scenes/showroom/ShowRoom.gd` |
| Assets Spine des ENNEMIS (08/2026) : `assets/personnages/ennemis/<id>/` — FlameBot + WorkBot livrés. MÊME contrat d'animation que Relic (`Idle`/`Attack_CaC`/`Hit`/`Death`), et les 5 PALIERS sont des **skins Spine** `<Nom>_Nv1`…`_Nv5` (Nv1 = Commun … Nv5 = Légendaire) commutées par `set_skin_by_name` + `set_slots_to_setup_pose` (sans le 2ᵉ appel la pose garde l'ancienne apparence). `creer()` prend skel/atlas/**apparence**/hauteur, Relic restant le défaut. Inspecter une livraison : `godot --headless --path . res://tools/inspect_spine_ennemis.tscn` (scène, même raison que verif_spine). Les apparences d'une entrée passent par `SpinePersonnagesData.apparences(entree, cosmetique)` : `variantes` (liste nommée) > `niveaux` (slots `_Nv<n>`) > `prefixe_skin` (5 paliers en skins) > apparence unique — un appelant n'a jamais à savoir laquelle ; il passe le Dictionary produit à `SpriteSpinePersonnage.definir_apparence()`. **RELIC — livraison « costumes » du 24/08/2026** : il n'a PAS une skin par apparence. Ses skins se CUMULENT (`skins_base` = corps `Men_Global` + équipement `Men_Level`/`Men_Level_Hit` + vêtement, plus UN jeu de `cosmetiques` — les accessoires de visage « Random », mutuellement exclusifs) en une skin composée (`SpineSprite.new_skin` + `add_skin`), et ses **6 paliers d'équipement sont portés par des SLOTS** suffixés `_Nv1`…`_Nv6` (`R_H_Idle_Tete_Nv3`, et des pièces mutualisées `…_Nv4/5`, `…_Nv1_2_3` — lecture : `SpriteSpinePersonnage.niveaux_du_slot`). Poser un niveau = PURGER la skin composée des slots des autres niveaux (`remove_attachment`) : vider les slots ne suffirait PAS, une animation qui repose un attachement le retrouverait dans la skin et rallumerait la pièce. Les versions masculine/féminine N'ONT PAS été livrées ; le jour où elles arrivent, remplir `variantes` dans le `.tres` suffit. **SENS D'EXPORT** (`regarde_a_droite`, 27/08/2026) : le registre décrit L'ASSET, l'appelant décide de la MISE EN SCÈNE. Christophe dessine CHAQUE CAMP PRÊT À L'EMPLOI — Relic vers la DROITE, FlameBot et WorkBot vers la GAUCHE : posés à leurs ancrages (joueur 0.25, adverse 0.75) ils se font face SANS AUCUN MIROIR. Le code appliquait pourtant un `scale = Vector2(-1, 1)` en dur à TOUT ennemi, ce qui les retournait DOS à Relic — bug signalé par Christophe, corrigé le 27/08/2026. ⚠ **Ne JAMAIS lire le sens sur l'ARME** : le FlameBot porte son canon en travers du corps, pointé vers l'arrière-droite, alors que sa TÊTE (masque en V, optiques) regarde à gauche — c'est cette confusion qui a produit le bug. Le REGARD donne le sens. Désormais `SpinePersonnagesData.echelle_x(entree, doit_regarder_a_droite)` donne le signe (+1 partout aujourd'hui) et `SpriteSpinePersonnage.orienter(signe)` ne touche que le SIGNE de `scale.x` (une taille posée par l'appelant survit). Une livraison qui changerait de sens = une ligne sur SON entrée, zéro code. ⚠ Les ennemis ne sont PAS encore branchés au combat (toujours des placeholders EnergyBoule) — à ce branchement, appeler `orienter()` comme le fait la ShowRoom. Le héros de combat est monté au **Nv1 (Commun, la dotation de départ)** par `SpriteSpinePersonnage.creer_heros(niveau)` : brancher le niveau RÉEL sur l'équipement du joueur se fait à cet appel, et nulle part ailleurs | `scripts/resources/SpinePersonnagesData.gd` |
...
- **Après CHAQUE livraison d'un PLAN DE VILLE, RE-BAKER LES TRACÉS DE NÉONS** :
  `godot --headless --path . --script res://tools/bake_neons_cite.gd` (~1 s ;
  `--headless` convient, l'outil ne lit que des images). Les tracés sont
  extraits de la DIFFÉRENCE entre la façade nue et la façade à enseignes : un
  plan relivré sans re-bake décollerait tous les points de leurs enseignes, en
  silence et joliment — le pire mode de panne. TestNeonsCite ÉCHOUE si le MD5
  d'un `.png` source ne correspond plus au bake ; au runtime, un cadrage qui a
  changé suffit à faire retirer les points (le décor reste correct, sans eux).
